...
try:
    ...
    import tkMessageBox
    tkMessageBox.showwarning('Warning!', 'Could not start the external menu for CreativeIT because wxPython is not installed.')
    sys.exit()
...
class CreativePanel(wx.Panel, ScriptClient):
    ...
    def OnHelp(self, event):
        try:
            import webbrowser
            webbrowser.open('http://nn.cs.utexas.edu/?creativeit08')
        except:
            print 'could not open help page for creativeit mod'
...
if __name__ == '__main__':
    app = wx.App(False)
    frame = wx.Frame(None, title="CreativeIT Controls", size=(600, 250))
    panel = CreativePanel(frame)
    frame.Show()
    app.MainLoop()
