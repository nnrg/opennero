...
class Fitness:
    """
    The single-objective fitness for each agent
    """
    HIT_TARGET = "Hit Target"
    dimensions = [HIT_TARGET]
    def __init__(self):
        self.data = {}
...
class NeroEnvironment(Environment):
    ...
    def __init__(self, XDIM, YDIM):
        from Vision.module import getMod
        """
        ...
        # actions
        abound.add_continuous(-pi / 2, pi / 2) # direction of NOT motion
        abound.add_continuous(0, 1) # how fast to NOT move
        abound.add_continuous(-pi / 2, pi / 2) # Firing direction
        abound.add_continuous(0, 1)
        abound.add_continuous(0, 1) 

        #sensors
        sbound.add_continuous(0, 1) # Section 1      
        sbound.add_continuous(0, 1) # Section 2
        sbound.add_continuous(0, 1) # Section 3
        sbound.add_continuous(0, 1) # Section 4
        sbound.add_continuous(0, 1) # Section 5
        sbound.add_continuous(0, 1) # Section 6
        sbound.add_continuous(0, 1) # Section 7
        sbound.add_continuous(0, 1) # Section 8
        sbound.add_continuous(0, 1) # Section 9
...
#        rbound.add_continuous(-100,100) # range for reward
    ...
    def getFriendFoe(self, agent):
        ...
        for r in self.teams:
            if r != myTeam:
                foe = self.teams[r] #NOT TODO MAKE THIS VIABLE OVER 3+ TEAMS
        return (friend, foe)
    ...
    def step(self, agent, action):
        ...
        #DON'T figure out the new x,y location
        ...
        #DON'T see if we can move forward
	...
	#calculate f
        ...
        # DON'T make the calculated motion
        ...
        #state.prev_pose = state.pose
        state.pose = (position.x, position.y, rotation.z)
	...
	getSimContext().getActiveCamera().setPosition(Vector3f(state.pose[0], state.pose[1], 10))
	getSimContext().getActiveCamera().setRotation(Vector3f(0, 0, state.pose[2]))
	
	image = cv.LoadImage("NERO/full-image.jpg", 0)
	edges = cv.CreateImage((image.width, image.height), image.depth, image.nChannels)
	cv.Canny(image, edges, 500, 600, 3)
	cv.SaveImage("NERO/edges.png", edges)

	h = edges.height
	w = int(edges.width/9)

	#Calculate average edges of vertical strips of view
	subsection = cv.GetSubRect(edges, (0, 0, w, h))
	vx.append((cv.Avg(subsection))[0]/255)
	subsection = cv.GetSubRect(edges, (w, 0, w, h))
	vx.append((cv.Avg(subsection))[0]/255)
	subsection = cv.GetSubRect(edges, (w*2, 0, w, h))
	vx.append((cv.Avg(subsection))[0]/255)
	subsection = cv.GetSubRect(edges, (w*3, 0, w, h))
	vx.append((cv.Avg(subsection))[0]/255)
	subsection = cv.GetSubRect(edges, (w*4, 0, w, h))
	vx.append((cv.Avg(subsection))[0]/255)
	subsection = cv.GetSubRect(edges, (w*5, 0, w, h))
	vx.append((cv.Avg(subsection))[0]/255)
	subsection = cv.GetSubRect(edges, (w*6, 0, w, h))
	vx.append((cv.Avg(subsection))[0]/255)
	subsection = cv.GetSubRect(edges, (w*7, 0, w, h))
	vx.append((cv.Avg(subsection))[0]/255)
	subsection = cv.GetSubRect(edges, (w*8, 0, w, h))
	vx.append((cv.Avg(subsection))[0]/255)
